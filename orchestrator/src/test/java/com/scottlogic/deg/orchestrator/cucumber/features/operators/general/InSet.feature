...
### inSet alone ###
  ...
  Scenario: Running an 'inSet' request that includes an invalid time value should fail with an error message
    ...
    And foo is in set:
      ...
      | 2017-12-31T23:59:59.999Z |
    Then the profile is invalid because "Field \[foo\]: Date string '2017-12-31T40:59:59.999Z' must be in ISO-8601 format: Either yyyy-MM-ddTHH:mm:ss.SSS\[Z\] between 0001-01-01T00:00:00.000Z and 9999-12-31T23:59:59.999Z or yyyy-mm-dd between 0001-01-01 and 9999-12-31"
    And no data is created
